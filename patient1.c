...
// get available options from the list
...
// loop through all the results and connect to the first we can
	...
	printf("Phase 1: The Patient1 has TCP port number %d and IP address %s.\n", (int) ntohs(localAddress.sin_port), inet_ntoa( localAddress.sin_addr));
        break;
}
...
if (p == NULL) {
 ...
 patientinfo_t *patient1 = (patientinfo_t*)malloc(sizeof(patientinfo_t));
    ...
    // Read patient1 username and password   
    FILE *file  = fopen("patient1.txt", "r");
    if(file == NULL) {
    ...
    return(-1);
      ...
      // Read patient2 username and password 
     file  = fopen("patient1insurance.txt", "r");
     if(file == NULL) {
     perror("Error opening file");
     return(-1);
     }
    ...
    {
         printf("Phase 1: Patient 1 authentication result: success.\n");
            ...
            printf("Phase 1: Patient1 authentication result: failure.\n");
            close(sockfd);
            exit(0);
    }
    ...
    memset(buf, 0, sizeof(buf));
    // Receive availability list from healthcenterserver
    if ((numbytes = recv(sockfd, buf, sizeof(buf), 0)) == -1) {
    perror("recv");
    exit(1);
    }

    // close socket and exit if not available
    ...
    printf("Please enter the preferred appointment index and press enter:");
     ...
     while(1){
            ...
            if(flag1)
                break;
            if(!flag2){
                printf("\nInvalid Selection. Please  enter  the  preferred  appointment  index  and  press  enter:");
                flag2 =1;
            }
            i=0;
        }
    ...
    // Receive appointment from healthcenterserver
    if ((numbytes = recv(sockfd, buf, sizeof(buf), 0)) == -1) {
    perror("recv");
    exit(1);
    }
   ...
   printf("Phase 3: The cost estimation request from Patient 1 with insurance plan %s has been sent to the doctor with port number %d and IP address %s.\n",patient1->insurance, port, inet_ntoa(servaddr.sin_addr));

   memset(buf, 0, sizeof(buf));

   // Receive cost estimation rom doctor
